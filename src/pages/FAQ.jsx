...
function FAQItem({ item }){
  ...
  return (
    <div key={item.id} id={item.id} className="collapsible">
      ...
      {open && (
        <div className="collapsible-content">
            ...
            <div style={{marginBottom:8}}>
              ...
              <ul>
                {item.links.map((l,idx)=> (
                  <li key={idx}><a href={l.href} target="_blank" rel="noreferrer">{l.label || l.href}</a></li>
                ))}
              </ul>
            </div>
          )}
          ...
          {item.note && (
            <div style={{color:'#b91c1c', marginTop:8}}>{item.note}</div>
          )}

          {item.images && item.images.length>0 && (
            <div style={{display:'grid', gap:8, gridTemplateColumns:'repeat(2,1fr)', marginTop:8}}>
              {item.images.map((im,idx)=>(
                <figure key={idx}>
                  <img src={im.src} style={{maxWidth:'100%', borderRadius:8, border:'1px solid #ddd'}}/>
                  <figcaption style={{fontSize:12, color:'#666'}}>{im.caption}</figcaption>
                </figure>
              ))}
            </div>
          )}
    ...
    <div style={{display:'grid', gap:16}}>
      ...
      <div className="collapsible no-print">
        <div className="collapsible-header">
          ...
          <span style={{fontSize:12, color:'#666'}}>Use search or tag cloud to filter</span>
        </div>
        <div className="collapsible-content">
          <input
            value={query}
