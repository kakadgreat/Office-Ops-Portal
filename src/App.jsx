...
export default function App(){
  ...
  React.useEffect(()=>{
    async function loadAll(){
      const paths = ['/data/offices.json','/data/meetings.json','/data/protocols.json','/data/departments.json','/data/events.json','/data/contacts.json','/data/advertising.json','/data/vendors.json','/data/tools.json','/data/calendars.json','/data/ithelp.json']
      const all = await Promise.all(paths.map(p=>fetch(p).then(r=>r.json())))
      setData({offices:all[0], meetings:all[1], protocols:all[2], departments:all[3], events:all[4], contacts:all[5], advertising:all[6], vendors:all[7], tools:all[8], calendars:all[9], ithelp:all[10]})
    }
    loadAll()
  },[])
  return (<div className='min-h-screen'>
    ...
    <footer className='border-t'><div className='max-w-6xl mx-auto px-4 py-6 text-sm text-gray-500'>Internal use only • Generated from Executive Coordinator Guide (sanitized)</div></footer>
  </div>)
}