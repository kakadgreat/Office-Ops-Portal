...
export default function App(){
  ...
  React.useEffect(()=>{
    async function loadAll(){
      const paths = ['/data/offices.json','/data/meetings.json','/data/departments.json','/data/events.json','/data/ithelp.json','/data/phone.json']
      const all = await Promise.all(paths.map(p=>fetch(p).then(r=>r.json().catch(()=>({})))))
      setData({offices:all[0], meetings:all[1], departments:all[2], events:all[3], ithelp:all[4], phone:all[5]})
    }
    loadAll()
  },[])
  return (<div className='min-h-screen'>
    <header className='sticky top-0 z-10 bg-white border-b'>
      <div className='max-w-6xl mx-auto px-4 py-3 flex items-center gap-4'>
        ...
        <nav className='flex flex-wrap gap-2 text-sm'>
          ...
          <NavItem to='/phone/edit' label='Edit Directory'/>
    ...
    <main className='max-w-6xl mx-auto px-4 py-6'>
      <Routes>
        ...
        <Route path='/calendars' element={<div/>}/>
      </Routes>
    </main>
    <footer className='border-t'><div className='max-w-6xl mx-auto px-4 py-6 text-sm text-gray-500'>PMG Ops Portal</div></footer>
  </div>)
}